...
program newtonMain
    ...
    nret = ScanForRoots(lb,ub,sw,eps,h,maxit,root,maxrt,iolog)

    ! Sorting arrays of zeros starting with the largest x
    if (nret>1) then
        call Sorting(root,nret,maxrt)
    end if
    ...
    write(*,*)'Output : '
    write(*,*)'--------------------------------------------'
    write(*,'(a,i15)')  'No. of roots found: ',nret
    write(*,*)'--------------------------------------------'
    write(iolog,*)'--------------------------------------------'
